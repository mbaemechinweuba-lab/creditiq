...
@st.cache_resource
def init_firebase():
    """Initialise Firebase using Streamlit secrets — credentials never exposed."""
    try:
...
def log_assessment(db, borrower_id, risk_label, default_prob, input_data):
    """Save assessment record to Firestore assessments collection."""
    if db is None:
        return
    try:
        ...
        pass  # Fail silently — logging should never block the assessment
...
if not run_btn:
    ...
    # ── Row 3: Risk Dimension Breakdown ───────────────────────────────────────
